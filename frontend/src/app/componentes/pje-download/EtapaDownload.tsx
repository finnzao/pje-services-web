...
export function EtapaDownload({
  ...
  const handleSubmit = () => {
    ...
    } else if (modo === 'by_number' && numerosParseados.length > 0) {
      onCriarJob({ mode: 'by_number', processNumbers: numerosParseados });
    }
  };
  ...
  const botaoLabel = (): string => {
    ...
    if (modo === 'by_number') return `Baixar ${numerosParseados.length} processo(s)`;
      ...
      {modo === 'by_number' && (
        <div>
          ...
          <div className="flex justify-between mt-2">
            <p className="text-xs text-slate-400">Separe por linha, vírgula ou ponto e vírgula. Máx: 500.</p>
