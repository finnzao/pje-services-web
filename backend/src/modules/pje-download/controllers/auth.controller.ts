...
export async function authRoutes(fastify: FastifyInstance) {
  fastify.post<{ Body: { cpf: string; password: string } }>('/login', async (request, reply) => {
    ...
    if (cpfDigits.length !== 11) {
      return reply.status(400).send({
        success: false,
        error: { code: 'INVALID_CPF', message: 'CPF deve ter 11 dígitos.', statusCode: 400 },
      });
    }
  ...
  fastify.post<{ Body: { sessionId: string; code: string } }>('/2fa', async (request, reply) => {
    ...
    if (!code || !/^\d{6}$/.test(code)) {
      return reply.status(400).send({
        success: false,
        error: { code: 'INVALID_CODE', message: 'Código 2FA deve ter 6 dígitos.', statusCode: 400 },
      });
    }
    if (!sessionId) {
      return reply.status(400).send({
  ...
  fastify.post<{ Body: { sessionId: string; profileIndex: number } }>('/profile', async (request, reply) => {
    ...
    if (profileIndex === undefined || profileIndex === null) {
      return reply.status(400).send({
        success: false,
        error: { code: 'MISSING_PARAMS', message: 'profileIndex obrigatório.', statusCode: 400 },
      });
    }
    if (!sessionId) {
      return reply.status(400).send({
    ...
    if (result.error) {
      ...
      return reply.status(sc).send({
        success: false,
        error: {
          code: isExpired ? 'SESSION_EXPIRED' : 'PROFILE_ERROR',
          message: isExpired ? 'Sessão PJE expirada.' : result.error,
          statusCode: sc,
        },
      });
    }
  ...
  fastify.get<{ Querystring: { sessionId: string } }>('/profiles', async (request, reply) => {
    ...
    if (result.error === 'SESSION_EXPIRED') {
      return reply.status(401).send({
        success: false,
        error: { code: 'SESSION_EXPIRED', message: 'Sessão PJE expirada.', statusCode: 401 },
      });
    }
